...
Function NewHttp(url As String, port=invalid As Dynamic, method="GET" As String) as Object
    ...
    this.GetToFileWithTimeout    = http_get_to_file_with_timeout
...
Function http_prep(method="" As String)
    ...
    urlobj.EnableFreshConnection(true)
...
Function http_get_to_string_with_retry(retries = 3) as String
...
Function http_get_to_string_with_timeout(seconds = 60) as String
    m.Prep("GET")
...
REM ******************************************************
REM
REM Performs Http.AsyncGetToFile() with a single timeout in seconds
REM To the outside world this appears as a synchronous API.
REM
REM Return empty string on timeout
REM
...
Function http_get_to_file_with_timeout(filename as String, seconds as Integer) as String
    m.Prep("GET")
    if (m.Http.AsyncGetToFile(filename)) then m.Wait(seconds)
    return m.response
End Function
